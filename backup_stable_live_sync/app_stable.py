...
with st.sidebar.expander("🔑 Login & AutoBet Settings", expanded=autobet_enabled):
    ...
    *(Jeetne par wapas ₹5 se start hoga, Haarne par next step)*
    ...
    st.markdown("---")
    with st.expander("⚡ 100% Guaranteed In-Browser AutoBet (Zero 403 Errors)", expanded=False):
        st.markdown("""
        **Cloudflare 403 Error Bypass:**
        Streamlit Cloud ke IP ko Daman block karta hai, lekin aapke browser me direct 100% chalega!
        
        👉 **Kaise Run Karein (30 Seconds):**
        1. Apne browser me `https://damanclub.in` par jayein (WinGo 30S kholen).
        2. Keyboard par **F12** dabakar **Console** tab me jayein.
        3. Niche diya gaya script paste karke **Enter** dabayein!
        """)
        st.code("""/**
